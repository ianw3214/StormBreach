...
Character Font::generateCharacter(const char character) {
    ...
    if(iter != m_characters.end()) {
        ...
        Character characterObj = {
            character,
            texture,
            Vec2i{static_cast<int>(m_fontFace->glyph->bitmap.width), static_cast<int>(m_fontFace->glyph->bitmap.rows)},
            Vec2i{static_cast<int>(m_fontFace->glyph->bitmap_left), static_cast<int>(m_fontFace->glyph->bitmap_top)},
