...
TextRenderer::TextRenderer(const std::string& fontPath, int size, Vec2i windowSize) :
    m_shader("res/shaders/text.vert", "res/shaders/text.frag"), m_fontSize(size), m_windowSize(windowSize){
...
void TextRenderer::render(std::string s, Vec2i pos, float scale) {
	renderLine(s, pos, scale, m_colour, m_ha, m_va);
}

void TextRenderer::renderLine(std::string s, Vec2i pos, float scale, Colour colour, TextRenderer::hAlign ha, TextRenderer::vAlign va) {
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

    m_shader.bind();
    m_shader.setUniform3f("textColour", colour.x, colour.y, colour.z);
    glActiveTexture(GL_TEXTURE0);
    glBindVertexArray(VAO);

	Vec2f sSize = computeLineSize(s, scale);
	GLfloat x_begin;
	switch (ha) {
...
Vec2f TextRenderer::computeLineSize(std::string s, float scale) {
	int x = 0;
	for (char c : s) {
		Character ch = m_text[c];
