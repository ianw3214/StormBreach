...
public:
    ...
    TextRenderer(const std::string& fontPath, int size, Vec2i windowSize);

	void render(std::string s, Vec2i pos, float scale = 1.f);
	inline void setAlignment(hAlign ha, vAlign va) { m_ha = ha; m_va = va; }
	inline void setColour(Colour colour) { m_colour = colour; }

private:

	void renderLine(std::string s, Vec2i pos, float scale, Colour colour = Colour{0, 0, 0}, hAlign ha = hAlign::left, vAlign va = vAlign::top);
    std::vector<Character> m_text;
	Vec2f computeLineSize(std::string s, float scale);
